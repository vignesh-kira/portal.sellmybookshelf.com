...
export function* advertisementFetchMyadsSaga(payload) {
	try {
		const result = yield call(advertisementFetchMyads, payload);
		if(!result.error){
			yield put(advertisementFetchMyadsSuccess(result.data));
		}else{
			yield put(advertisementFetchMyadsError());
		}
	} catch (e) {
		yield put(advertisementFetchMyadsError());
	}
